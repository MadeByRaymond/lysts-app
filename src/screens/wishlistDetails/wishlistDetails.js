...
export default class wishlistDetails extends Component {
    ...
    state={
        ...
        noData: {value:true, message: 'Wishlist doesn\'t exist'},
        actionsModal : false,
    ...
    componentWillUnmount() {
        if(this.realm != null || this.realm != undefined || this.realm != ""){
    ...
    openRealm = () => {
        ...
        //   realm = await Realm.open(config);
          Realm.open(config).progress((d1, d2) => {
            ...
            if (wishlistData.length < 1) { this.setState({noData: {value:true, message: "Wishlist doesn't exist"}}) } 
            else if (wishlistData[0].status && wishlistData[0].status != 'active') {  this.setState({noData: {value:true, message: "This wishlist is no longer active"}}) } 
            else {
                let ownerData = (
                ...
                if (ownerData.length < 1){ this.setState({noData: {value:true, message: "This wishlist no longer exists"}}) }
                else {
                    this.setState({
                        owner: (this.user.id == wishlistData[0].owner) ? true : false,
                        loading: false,
                        wishlistInfo: {
                            ...
                            listItems: wishlistData[0].listItems,
                            owner: (
                                (ownerData[0].displayName == undefined || ownerData[0].displayName == null || ownerData[0].displayName.trim() == "") 
                                ? `${ownerData[0].firstName.trim().trimStart()} ${ownerData[0].lastName.trim().trimStart()}`.trim() 
                                : ownerData[0].displayName.trim().trimStart())
                        }
                    })
                }
            }
    ...
    showWishlistDetails = () =>{
        ...
        : (
            <View style={styles.container}>
                ...
                <View style={{flex: 1, paddingTop: ((dHeight/2.8) + 35 + 30)}}>
                    <View style={[styles.contentWrapper, showTopBar ? styles.contentWrapperWithTopBar : null]}>
                        <View style={styles.topWrapper}>
                            <View style={{maxWidth: dWidth - 60 - 70,}}>
                                <View style={styles.name}><Text style={styles.nameText}>No Data</Text></View>
                            </View>
                            <View>
                                <Image 
                                    style={styles.imageAvatar}
    ...
    render() {
        ...
        return this.state.loading ? (
            <View style={{
                ...
                <View>
                    <LottieView 
                        style={{width: '100%', height: 250, marginTop: 0, alignSelf: 'center',backgroundColor:'transparent'}}
